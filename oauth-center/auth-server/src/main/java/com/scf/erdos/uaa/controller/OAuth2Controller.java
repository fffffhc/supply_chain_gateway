...
import com.scf.erdos.common.token.SmsCodeAuthenticationToken;
...
import com.scf.erdos.uaa.utils.SpringUtil;
...
OAuth2Controller {
	...
	@DeleteMapping(value = "/oauth/remove/token/{access_token}")
	@LogAnnotation(module = "auth-server", recordRequestParam = false)
	public Result removeToken(@PathVariable String access_token) {
		OAuth2AccessToken accessToken = tokenStore.readAccessToken(access_token);
		if (accessToken != null) {
			// 移除access_token
			tokenStore.removeAccessToken(accessToken);
	...
	@GetMapping("/oauth/token/list")
	@LogAnnotation(module = "auth-server", recordRequestParam = false)
	public Result getUserTokenInfo(@RequestParam Map<String, Object> params)
			throws Exception {
		...
		PageResult pageResult = PageResult.builder().page(page).limit(limit).data(list).count((long)list.size()).build();
